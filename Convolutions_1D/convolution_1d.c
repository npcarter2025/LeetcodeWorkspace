...
int* convolution_1d(int* mat, int* kernel, int matSize, int kernelSize, int* returnSize) {
    ...
    *returnSize = elems;
    ...
    for (int i = 0; i < elems; i++) {
        for (int j = 0; j < kernelSize; j++) {
            convoluted[i] += mat[i+j] * kernel[kernelSize - 1-j];   
        }
    }
    return convoluted;
}
...
int main(int argc, char* argv[]) {
    ...
    convoluted = convolution_1d(matrix, kernel, mat_size, k_size, &returnSize);
