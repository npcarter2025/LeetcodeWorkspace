...
int** convolution_2d(int** mat, int** kernel, int matSize, int kernelSize, int* returnSize, int** returnColumnSizes) {
        ...
        for (int b = 0; b < width; b++) {
            ...
            for (int x = 0; x < kernelSize; x++ ) {
                for (int y = 0; y < kernelSize; y++) {
                    correlated[a][b] += mat[x+a][y+b] * kernel[kernelSize - 1 - x][kernelSize - 1 - y];
                }
            }
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    correlated = convolution_2d(matrix, kernel, mat_size, k_size, &returnSize, &returnColumnSizes);
